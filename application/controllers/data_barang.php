...
class Data_barang extends OperatorController {
	...
	public function index() {
		...
		$crud->columns('nm_barang','kode','type','merk','harga','jml_brg','ket');
		$crud->set_subject('Data Barang');
	
		$crud->fields('nm_barang','kode','type','merk','harga','jml_brg','ket');
		$crud->set_rules('kode','kode','max=5');
		$crud->display_as('kode','Kode (maksimum 5)');
		$crud->display_as('nm_barang','Nama Barang');
		$crud->display_as('jml_brg','Jumlah Barang');
		
		$crud->required_fields('nm_barang','kode','harga','jml_brg');
