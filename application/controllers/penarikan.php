...
class Penarikan extends OperatorController {
	...
	function cetak_laporan() {
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">LAPORAN TRANSAKSI PENARIKAN SIMPANAN TUNAI <br></span>
			<span> PERIODE : '.jin_date_ina($tgl_dari).' S/D '.jin_date_ina($tgl_sampai).'</span> 
		...
		<table width="100%" cellspacing="0" cellpadding="3" border="1" border-collapse= "collapse">
			...
			$grand_total = 0;
			foreach ($jns_simpan_loop as $key => $tarik) {
			$jml_penarikan[$key] = 0;
				if (!empty($this->penarikan_m->lap_data_penarikan($tarik->id))) {
				// print_r($this->penarikan_m->lap_data_penarikan($tarik->id));
					foreach ($this->penarikan_m->lap_data_penarikan($tarik->id) as $row) {
					...
					<tr>
						...
						<td class="h_kanan"> <strong>'.number_format($jml_penarikan[$key]).'</strong></td>
					</tr>';
				}
			}
		$html .= '
