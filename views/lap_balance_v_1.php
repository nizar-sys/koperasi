	...
	<div class="col-md-12">
		<div class="box box-solid box-primary">
			...
			<div style="padding: 25px;">
				<table id="" border="1" class="display nowrap" style="width:100%">
					<thead>
						<tr>
					...
					<tbody>
						...
						foreach ($akun_balance as $key => $ak) {
							...
							<?php
							if ($key < 64) {
								$akun_before = $akun_balance[$key + 1]->vcCOACode;
								$akun = $akun_balance[$key]->vcCOACode;
								$kode_before = explode('.', $akun_before);
								$kode = explode('.', $akun);
								if ($ak->itCOALevel == '2') {
									if ($kode[1] != $kode_before[1]) {
										?>
										<tr>
											<td></td>
											<td class="text-left">
												<?php
												for ($i = 0; $i<=$ak->itCOALevel-1; $i++) {
													echo "&emsp;&emsp;";
												}
												?>
												<b>Sub Total&emsp;&emsp;:</b>
											</td>
											<td></td>
											<td></td>
											<td></td>
								...
								}elseif($ak->itCOALevel == '3'){
									if($kode[2] != $kode_before[2]){
										?>
										<tr>
											<td></td>
											<td class="text-left">
