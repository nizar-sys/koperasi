			...
			<div style="padding: 25px;">
				<table id="example" border="" class="display nowrap" style="width:100%">
					...
					<tbody>
						...
						$total = 0;
						$total_hasil = 0;
						foreach ($akun_laba as $key => $ak) {
							$code = explode('.', $ak->vcCOACode);
							if($code[0] == '33'){
								$data_before = $jurnal_umum_m->get_data_laba_before_by_id($ak->vcCOACode)->result();
								$credit_before = 0;
								$hasil_before = 0;
								foreach ($data_before as $dat_bef) {
									$credit_before += $dat_bef->cuJournalCredit;
								}
								$hasil_before = $credit_before;

								$data = $jurnal_umum_m->get_data_laba_by_id($ak->vcCOACode)->result();
								$credit = 0;
								$hasil = 0;
								foreach ($data as $bal) {
						...
						<tr>
							<td></td>
							<td class="text-left"><b>&emsp;&emsp;Total Pendapatan</b></td>
						...
						foreach ($akun_laba as $key => $ak) {
							...
							if($code[0] == '44'){
								...
								$hasil_before = 0;
								foreach ($data_before as $dat_bef) {
									$debet_before += $dat_bef->cuJournalDebet;
								}
								$hasil_before = $debet_before;

								$data = $jurnal_umum_m->get_data_laba_by_id($ak->vcCOACode)->result();
								$debet = 0;
								$hasil = 0;
								foreach ($data as $bal) {
									$debet += $bal->cuJournalDebet;
								}
								$hasil = $debet;
								?>
								<tr>
								...
								<?php
								$total_before_biaya += $hasil_before;
								$total_biaya += $hasil;
								$total_hasil_biaya += $hasil_before - $hasil;
							}
						}
						?>
						<tr>
							<td></td>
							<td class="text-left"><b>&emsp;&emsp;Total Biaya</b></td>
							<td class="text-right"><b><?php echo "Rp" . number_format($total_before_biaya, 2, ',', '.'); ?></b></td>
							<td class="text-right"><b><?php echo "Rp" . number_format($total_biaya, 2, ',', '.'); ?></b></td>
							<td class="text-right"><b><?php echo "Rp" . number_format($total_hasil_biaya, 2, ',', '.'); ?></b></td>
						</tr>
					</tbody>
					<tfoot>
						<td></td>
						<td class="text-center"><b>Total Laba/Rugi</b></td>
						<td class="text-right"><b><?php echo "Rp" . number_format(($total_before - $total_before_biaya), 2, ',', '.'); ?></b></td>
						<td class="text-right"><b><?php echo "Rp" . number_format(($total - $total_biaya), 2, ',', '.'); ?></b></td>
						<td class="text-right"><b><?php echo "Rp" . number_format(($total_hasil - $total_hasil_biaya), 2, ',', '.'); ?></b></td>
					</tfoot>
				</table>
			</div>
		</div>
